...
import { PlatformType } from './typing.js';
...
const uploadZipFile = async (token, config, filePath) => {
...
const updateVersion = async (token, config) => {
    ...
    if (isMobile) {
        ...
        archive.finalize();
        output.on('close', () => {
            console.log(chalk.green(tagName, `Compression completed, file size: ${archive.pointer()} bytes`));
            uploadZipFile(token, config, output.path);
        });
    }
};
const updateAction = async (name) => {
    ...
    const [err, res] = await to(explorer.load(join(process.cwd(), 'updater-cli.config.ts')));
    if (err || !res) {
